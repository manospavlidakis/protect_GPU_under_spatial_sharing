...
void getKernels(std::string ptx_file, Kernels &kernels) {
...
void help(char *argv[]) {
  ...
  std::cerr << "\t" << argv[0]
            << " <executable> <output_dir> [optional cuobjdump executable]"
            << std::endl
            << std::endl;
  std::cerr << "Extracts ptx and kernel list from <executable>, and places "
...
int main(int argc, char *argv[]) {
  ...
  //getPtxFiles(obj, ptx_files);
  std::string directoryPath = "ptx_pytorch_keep_nat";
  getPtxFilesFromDirectory(directoryPath, ptx_files);

  std::cerr << "Found " << ptx_files.size() << " modules in " << obj
            << std::endl;
