...
static uint64_t s_index_map1;
static uint64_t e_index_map1;
...
std::unordered_map<std::string, CUmodule> createModuleMap(
  ...
  for (const auto &entry : PTXMap) {
    ...
    std::string path = PATH"ptx_pytorch_mod/" + ptx;
    std::cerr<<"PTX: "<<path<<std::endl;
...
void parseJson(
  ...
  std::cerr << "-> 2xinstructions!! " << std::endl; 
  // std::cerr<<__func__<<std::endl;
  std::string jsonFile = PATH"/pytorch_klist.json";
