...
cudaError_t cudaMalloc(void **devPtr, size_t size) {
  ...
  if (size < partitions[partition_index].partition_free_space) {
    ...
    *devPtr = allocation.base_alloc_ptr;
    ...
    std::cerr << "Allocation-> Lowest addr (Devive ptr): "
              << allocation.base_alloc_ptr << " , Size: " << size
              << " , Allocations num: "
              << partitions[partition_index].allocation_cnt - 1
              << " , Next allocation will start from: "
              << prev_end_ptr
              //                <<" , Rounded next allocation will start from: "
              //		<< align(128*1024, prev_end_ptr)
              << " , Mask: " << std::hex << partitions[partition_index].mask
              << std::dec
              //<<" , Devive ptr: "<<*devPtr
              << std::endl;
...
std::unordered_map<std::string, CUmodule> createModuleMap(
  ...
  for (const auto &entry : PTXMap) {
    ...
    std::string path = PATH "/mod_caffe_libs/" + ptx;
    ...
    if (err != CUDA_SUCCESS) {
      ...
      std::cerr << RED << " cuModuleLoadData error : " << RESET << err_str
                << " " << err << " for PTX: " << my_ptx.c_str() << std::endl;
      abort();
    }
...
// #endif
  ...
  /*  if (firstcudaLaunch == 0) {
    static unsigned long long kernelCount
  ...
  */
  ...
  //  printf("Intercepted launch of kernel %p with grid size %d x %d x %d and
  //  block size %d x %d x %d - NEW KRNL: %p\n", func, gridDim.x, gridDim.y,
  //  gridDim.z, blockDim.x, blockDim.y, blockDim.z, kernelFunc);
  ...
  newArgs[argsNum] = (void *)&deviceExtraArgument;
#ifdef DEBUG
  std::cerr << std::hex << "Mask: " << deviceExtraArgument << std::dec
            << " newArgs[argsNum]: " << newArgs[argsNum] << std::endl;
#endif
