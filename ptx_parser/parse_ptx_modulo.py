...
def load_mask(ptx_file, out_ptx):
	...
	for line in ptx:
			#print("Line: "+line)
			if '//Partition base' in line: 
				#print("Line: "+line)
				tmp_reg = line.split()
				my_reg_low = tmp_reg[1].replace(",","")
			if '//Partition size' in line:
				#print("Line: "+line)
				tmp_reg1 = line.split()
				my_reg_high = tmp_reg1[1].replace(",","")
				#print("Reg: "+str(my_reg_high))
...
#				load_instructions.append(line)
				...
				if (character == ''):
					# modulo
					out_ptx.write("\tsub.u64 \t %maskreg3, " + concat_reg + ", " + my_reg_low +";\n")
					out_ptx.write("\tcvt.u32.u64 \t %maskp1, %maskreg3 ;\n")
					out_ptx.write("\tcvt.u32.u64 \t %maskp2, %maskreg2 ;\n")
					out_ptx.write("\trem.u32 \t %maskp1, %maskp2, %maskp2 ;\n")
					out_ptx.write("\tcvt.u64.u32 \t %maskreg3, %maskp1 ;\n")
					out_ptx.write("\tadd.u64 \t"+ concat_reg + "%maskreg1, %maskreg3;\n")
				...
				# if there is +, - in ld and st instructions
					...
					out_ptx.write("\tadd.s64 \t" + without_arithm + ", "
						+ without_arithm
						+ ", "+ number + "; //+or- mask\n")
					out_ptx.write("\tsub.u64 \t %maskreg3, " + without_arithm + ", " + my_reg_low +";\n")
					out_ptx.write("\tcvt.u32.u64 \t %maskp1, %maskreg3 ;\n")
					out_ptx.write("\tcvt.u32.u64 \t %maskp2, %maskreg2 ;\n")
					out_ptx.write("\trem.u32 \t %maskp1, %maskp2, %maskp2 ;\n")
					out_ptx.write("\tcvt.u64.u32 \t %maskreg3, %maskp1 ;\n")
					out_ptx.write("\tadd.u64 \t"+ without_arithm + "%maskreg1, %maskreg3;\n")
...
#				load_instructions.append(line)
			...
			# store 
				...
				#print ("C: "+ character)
				if (character == ''):
					# append to the new ptx the bitwise operation
					out_ptx.write("\tsub.u64 \t %maskreg3, " + concat_reg + ", " + my_reg_low +";\n")
					out_ptx.write("\tcvt.u32.u64 \t %maskp1, %maskreg3 ;\n")
					out_ptx.write("\tcvt.u32.u64 \t %maskp2, %maskreg2 ;\n")
					out_ptx.write("\trem.u32 \t %maskp1, %maskp2, %maskp2 ;\n")
		...
		for line in ptx:
			...
			# Add the extra parameter
				...
				if found_reg == 1:
					...
					new_registers.append("\t.reg .u64 \t%maskreg<4>;\n")
					new_registers.append("\t.reg .u32 \t%maskp<3>;\n")
				...
				if first == 1:
					...
					new_lines.append("\tld.param.u64\t%maskreg1, ["
						...
						+str(par_count)+"];"+"   //Partition base"+"\n")
					new_lines.append("\tld.param.u64\t%maskreg2, ["
						+str(current_kernel)+"_param_"
						+str(par_count)+"];"+"   //Partition size"+"\n")
				...
				if 'ld.param.' not in line:
					...
					if first == 1:
						...
						new_lines.append("\tld.param.u64\t%maskreg2, ["
							+str(current_kernel)+"_param_"
							+str(par_count+1)+"];"+"   //Partition size"+"\n")
						out_ptx.writelines(new_lines)
						first = 0
					else:
						out_ptx.write(line)
